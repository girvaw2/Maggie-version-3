/*
 * maggie_openrave_kinematics.cpp
 * Based upon the maggie implementation by Henning Deeken <hdeeken@uos.de>
 */
...
{
  ros::init(argc, argv, "maggie_openrave_kinematics");
  ros::NodeHandle n;
  ...
  if (!n.getParam("maggie_joints", joint_names))
  {
  ...
  ROS_INFO("maggie_kinematics_constraint_aware sucessfully established it's services");
  for (size_t i = 0; i < 5; ++i)
